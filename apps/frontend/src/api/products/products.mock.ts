...
export const getMockedProductOverview = () => {
  return new Promise<OverviewProduct[]>((resolve) =>
    resolve(mockedProductOverview)
  );
};

export const getMockedProductdetails = (id: number) => {
  return new Promise<DetailedProduct | null>((resolve) => {
    const product = mockedProuctDetailedOverview.find((product) => product.id === id);
